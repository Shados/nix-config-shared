...
let
  inherit (lib) escapeShellArg mkOption optionalAttrs singleton types;
...
in
{
  ...
  in {
    files = {
      ...
      "/usr/bin/replace-root-password".source = compileMoonBin "replace-root-password" ./replace-root-password.moon;
      "/usr/bin/replace-root-password".mode = "0755";
    }
    // optionalAttrs (config.sshHostEd25519Key != null) {
      "/etc/dropbear/dropbear_ed25519_host_key".source = pkgs.runCommandNoCCLocal "openwrt-ed25519-key" {
        nativeBuildInputs = [
          pkgs.dropbear
        ];
