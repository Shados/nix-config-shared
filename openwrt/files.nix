...
let
  inherit (lib) mkDefault mkDerivedConfig mkIf mkOption singleton types;
in
{
  options = {
    # TODO assertions
    files = mkOption {
      type = types.attrsOf (types.submodule ({ name, config, options, ... }: {
        options = {
          ...
          type = mkOption {
            type = types.enum [ "file" "directory" "symlink" ];
            default = "file";
            description = ''
        ...
        config = {
          ...
          source = mkIf (config.text != null) (
            let name' = "openwrt-" + baseNameOf name;
  ...
  config = let
    fileTree = pkgs.runCommandNoCCLocal "openwrt-file-tree" {
      nativeBuildInputs = [
        (pkgs.luajit.withPackages(p: with p; [ moonscript rapidjson inspect luv ]))
      ];
