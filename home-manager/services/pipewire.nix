...
{ config, lib, pkgs, ... }:
let
  inherit (lib) attrNames concatMapStringsSep flip mapAttrs' mkEnableOption mkIf mkOption nameValuePair types;
...
in
{
  ...
  config = mkIf cfg.enable ({
    ...
    xdg.configFile."pipewire/pipewire.conf.d/" = let
      mkConfFileName = overrideName: "home-manager_${overrideName}.conf";
    in {
      source = pkgs.runCommandNoCC "pipewire-config-overrides" {
          preferLocalBuild = true;
          nativeBuildInputs = [ pkgs.jq ];
          confFiles = builtins.toJSON
            (flip mapAttrs' cfg.configOverrides
