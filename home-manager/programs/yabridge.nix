{ config, lib, pkgs, ... }:
let
  inherit (lib) concatMapStringsSep escapeShellArg escapeShellArgs mapAttrsToList mkEnableOption mkIf mkOption nameValuePair singleton types;
  ...
  config = mkIf cfg.enable {
    xdg.dataFile.${pluginDirName} = {
      ...
      source = pkgs.symlinkJoin {
        ...
        paths = cfg.pluginDirs ++ singleton (pkgs.runCommandNoCC "hm-yabridge-pluginfiles-dir" {
    ...
    home.packages = [ cfg.package cfg.packageCtl ];
    home.activation.yabridge-sync = let
      jq = "${pkgs.jq}/bin/jq";
