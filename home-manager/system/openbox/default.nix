...
let
  ...
  upperCaseActions =
    actionList: flip map actionList (action: action // { action = upperCaseFirst action.action; });
  ...
  actionNameCheck = action: actionNameType.check (if isAttrs action then action.action else action);
  ...
  options = {
    xsession.windowManager.openbox = {
      ...
      keyboard = {
        ...
        keybind = mkOption {
          type = with types; nameCheckAttrsOf (x: keyComboType.check x) (listOf actionType);
          default = { };
          # TODO example
      ...
      theme = {
        ...
        fonts = {
          activeWindow = mkFontOption "title bar of an active window" {
            size = 8;
            weight = "bold";
          };
          inactiveWindow = mkFontOption "title bar of an inactive window" {
            size = 8;
            weight = "bold";
          };
          menuHeader = mkFontOption "titles in the right-click menu" { };
          menuItem = mkFontOption "items in the right-click menu" { };
          activeOnScreenDisplay =
            mkFontOption "text in active popups (e.g. window cycling or desktop switching popups)"
              { weight = "bold"; };
          inactiveOnScreenDisplay =
            mkFontOption "text in inactive popups (e.g. window cycling or desktop switching popups)"
              { weight = "bold"; };
        };
      };
  ...
  config = mkIf cfg.enable {
    ...
    xsession.windowManager.command =
      let
        ...
        cmd =
          with pkgs;
          writeShellScript "openbox-startup.sh" ''
    ...
    xsession.windowManager.openbox.transformedConfig =
      let
        transformedConfig = strippedConfig // {
          ...
          theme = strippedConfig.theme // {
            fonts = flip mapAttrs' strippedConfig.theme.fonts (
              name: value: nameValuePair (upperCaseFirst name) value
            );
          };
        };
        transformApp =
          ...
          // {
            ...
            groupclass = app.groupClass;
            skip_pager = app.skipPager;
            skip_taskbar = app.skipTaskbar;
          };
        transformMousebind =
          name: value:
          nameValuePair (concatStringsSep " " (map (upperCaseFirst) (splitString " " name))) value;
    ...
    xsession.windowManager.openbox.finalConfigFile =
      pkgs.runCommand "rc.xml"
        {
