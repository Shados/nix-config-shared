...
in
{
  config = mkMerge [
    ...
    (mkIf (config.services.screen-locker.enable && config.xsession.windowManager.openbox.enable) {
      xsession.windowManager.openbox.keyboard.keybind."W-A-l" = lib.singleton {
        action = "execute";
    ...
    (
      ...
      mkIf config.services.screen-locker.enable {
        ...
        home.activation.schedule-unlock-only-restarts =
          let
            configHome = lib.removePrefix config.home.homeDirectory config.xdg.configHome;
            systemctl = lib.getExe' pkgs.systemd "systemctl";
          in
          lib.hm.dag.entryBetween
            [ "performServiceRestarts" ]
            [ "onFilesChange" "reloadSystemd" "initServiceRestartArray" ]
            ''
