...
in
{
  config = mkMerge [
    (mkIf config.services.mullvad-vpn.enable {
      ...
      systemd.services.mullvad-daemon.serviceConfig.ExecStart =
        lib.mkForce "${config.services.mullvad-vpn.package}/bin/mullvad-daemon --disable-log-to-file --disable-stdout-timestamps";
